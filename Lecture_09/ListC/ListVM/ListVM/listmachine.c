...
void mark(word* block) {
    // Is block nil (for car and cdr), already marked, or not in heap
    if (block == 0 || !inHeap(block) || Color(block[0]) != White) return;

    if (Color(block[0]) == Black) return; // Already marked

    block[0] = Paint(block[0], Grey); // being processed

    unsigned int tag = BlockTag(block[0]);

    if (tag == CONSTAG) {
        // if car and cdr are heap references and not nil
...
void markPhase(word s[], word sp) {
    ...
    // Set all non-Blue blocks to white, so they can be marked
    word* heapPtr = heap;
    while (heapPtr < afterHeap) {
        if (Color(heapPtr[0]) != Blue) {
    ...
    for (word i = 0; i <= sp; i++) {
        if (!IsInt(s[i]) && s[i] != 0) { // Is heap refernce and not nil
            mark((word*)s[i]);
        }
    }
}
...
void sweepPhase() {
  ...
  word* heapPtr = heap;
  while (heapPtr < afterHeap) {
      uword length = Length(heapPtr[0]);
      unsigned int color = Color(heapPtr[0]);

      word* nextBlock = heapPtr + length + 1;

      // Paint white blocks blue
      if (color == White) {
          word* current = heapPtr;
          uword totalLength = length;

          // Check if any of the following blocks are white
          while (nextBlock < afterHeap) {
              uword nextLength = Length(nextBlock[0]);
              unsigned int nextColor = Color(nextBlock[0]);

              if (nextColor == White) {
                  totalLength += nextLength + 1;
          ...
          if (totalLength > 0) {
              current[0] = mkheader(0, totalLength, Blue);
              current[1] = (word)freelist;
              freelist = (word)current;
          }
      }
